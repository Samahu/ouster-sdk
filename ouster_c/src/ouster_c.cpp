...
#include "ouster/types.h"     // SensorInfo, PacketFormat

namespace sensor = ouster::sdk::sensor;
namespace core = ouster::sdk::core;

struct ouster_client {
    std::shared_ptr<sensor::Client> cli;        // underlying C++ client
    std::string metadata;                       // cached metadata json
    std::unique_ptr<core::SensorInfo> info;     // parsed info
    std::unique_ptr<core::PacketFormat> pf;     // packet format
};
...
ouster_client_t* ouster_client_create(const char* hostname, int lidar_port,
    ...
    try {
        auto cpp_cli = sensor::init_client(std::string(hostname),
                                                   lidar_port, imu_port);
...
int ouster_client_poll(ouster_client_t* client, int timeout_sec) {
    ...
    try {
        auto state = sensor::poll_client(*client->cli, timeout_sec);
...
int ouster_client_get_metadata(ouster_client_t* client, char* buffer,
    ...
    try {
        if (client->metadata.empty()) {
            client->metadata = sensor::get_metadata(*client->cli);
        }
...
int ouster_client_fetch_and_parse_metadata(ouster_client_t* client,
    ...
    try {
        client->metadata = sensor::get_metadata(*client->cli, timeout_sec);
        core::ValidatorIssues issues;
        nonstd::optional<core::SensorInfo> si_opt;
        bool ok = core::parse_and_validate_metadata(client->metadata, si_opt,
                                                    issues);
        if (!ok || !si_opt.has_value()) {
            return set_error_and_return(-2);
        }
        client->info = std::make_unique<core::SensorInfo>(si_opt.value());
        client->pf = std::make_unique<core::PacketFormat>(*client->info);
        return 0;
    } catch (...) {
...
int ouster_client_read_lidar_packet(ouster_client_t* client, uint8_t* buf,
    ...
    try {
        bool ok = sensor::read_lidar_packet(*client->cli, buf, needed);
        return ok ? 1 : 0;
    } catch (...) {
        return 0;
    }
...
int ouster_client_read_imu_packet(ouster_client_t* client, uint8_t* buf,
    ...
    if (buf_size < needed) return 0;
    try {
        bool ok = sensor::read_imu_packet(*client->cli, buf, needed);
        return ok ? 1 : 0;
    } catch (...) {
        return 0;
    }
...
int ouster_client_get_lidar_port(const ouster_client_t* client) {
    if (!client || !client->cli) return -1;
    try {
        return sensor::get_lidar_port(*client->cli);
    } catch (...) {
        return -1;
    }
}
...
int ouster_client_get_imu_port(const ouster_client_t* client) {
    if (!client || !client->cli) return -1;
    try {
        return sensor::get_imu_port(*client->cli);
    } catch (...) {
        return -1;
    }
}
